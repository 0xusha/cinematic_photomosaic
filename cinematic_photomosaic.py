...
def run_photomosaic_experience(image_path, grid_cols=80, grid_rows=80):
    ...
    # Artificially boost the brightness and contrast of the entire source image
    img = cv2.convertScaleAbs(img, alpha=1.15, beta=30)

    # Get screen resolution dynamically first to prevent image from exceeding screen bounds
    root = tk.Tk()
    screen_w = root.winfo_screenwidth()
    screen_h = root.winfo_screenheight()
    root.destroy()

    # Resize canvas based on screen dimensions to prevent out-of-bounds slicing errors
    ...
    # 2. Precompute the static tile images ONLY ONCE to save CPU during the loop
    print("Precomputing mosaic tiles...")
    static_tiles = []
    ...
    # Calculate offsets to center the animation on the black canvas
    ...
    # Helper function to center frames on the black screen
    ...
    # ===================================================================
    while True:
        # FIXED: Drastically reduced flight distance and delay to eliminate the long black gap
        active_tiles = []
        max_delay = 15 
        for x, y, tinted in static_tiles:
            angle = random.uniform(0, 2 * math.pi)
            # Tighter radius so they don't spend seconds completely off-screen
        ...
        # -------------------------------------------------------------------
        # PHASE 1: Cinematic Swarm Assembly (Ease-Out)
        # -------------------------------------------------------------------
        print("Starting Cinematic Swarm Assembly...")
        # FIXED: Sped up the swarm assembly to make the loop punchier
        swarm_frames = 75 
        ...
        # -------------------------------------------------------------------
        # PHASE 2: Sub-Pixel Smooth Zoom IN
        # -------------------------------------------------------------------
        print("Starting Smooth Zoom IN...")
        ...
        # -------------------------------------------------------------------
        # PHASE 3: Sub-Pixel Smooth Zoom OUT
        # -------------------------------------------------------------------
        print("Starting Smooth Zoom OUT...")
        ...
        # -------------------------------------------------------------------
        # PHASE 4: Swarm Dispersal
        # -------------------------------------------------------------------
        print("Blowing away to reset loop...")
        for f in range(swarm_frames + 1):
            frame = np.zeros((h, w, 3), dtype=np.uint8)
            ...
            for t_data in active_tiles:
                ...
                # FIXED: Updated to match the new max_delay dynamically
